...
int main(int argc, char* argv[]) {
    ...
    for (int i = LOWER; i <= UPPER; i++) {
        ...
        int result = connect(soc, (struct sockaddr*) &peer, sizeof(peer));
        if (result == -1) {
            fprintf(stderr, "Port %d is not responding", i);
        }
        else if (write(soc, buf, BLOCK_SIZE) == -1) {
            fprintf(stderr, "Port %d is not listening", i);
        } 
        else if (read(soc, buf2, BLOCK_SIZE) == -1) {
            fprintf(stderr, "Port %d is not responding", i);
        } 
        else if (strncmp(buf2, "HTTP", strlen("HTTP"))) {
            printf("Port %d answered with HTTP", i);
        } 
        else {
            fprintf(stderr, "Port %d is not a web server", i);
        }
    }
